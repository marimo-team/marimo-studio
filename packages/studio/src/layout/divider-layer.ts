...
export class DividerLayer {
  ...
  render(dividers: DividerRectangle[]): void {
    ...
    for (const divider of dividers) {
      ...
      element.setAttribute("aria-orientation", divider.axis === "x" ? "vertical" : "horizontal");
      element.setAttribute("aria-label", divider.axis === "x" ? "Resize columns" : "Resize rows");
      element.setAttribute("aria-valuemin", "10");
      element.setAttribute("aria-valuemax", "90");
      element.setAttribute("aria-valuenow", String(Math.round(divider.ratio * 100)));
  ...
  private bind(element: HTMLElement, id: string): void {
    ...
    element.addEventListener("keydown", (event) => {
      ...
      this.commit(
        updateRatio(this.tree(), id, divider.ratio + (event.key === positive ? step : -step)),
      );
    });
  }
