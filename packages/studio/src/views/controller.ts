...
export class ViewController {
  ...
  private async create(name: string): Promise<void> {
    ...
    try {
      if (!(await this.prepareCurrentView())) {
        this.menu.showCreateMessage("Resolve the current source before creating a view.", "error");
        return;
      }
  ...
  private async deleteSelected(): Promise<void> {
    ...
    try {
      if (name === this.current && !(await this.prepareCurrentView())) {
        this.menu.showRemovalError("Resolve the current source before removing this view.");
        return;
      }
  ...
  private render(): void {
    this.menu.render(this.current, this.views, this.removing, this.deleting);
  }
