import { notifyPageTheme } from "./theme.ts";
...
const PAGE_STYLE_SELECTOR =
  `link[rel="stylesheet"][${PAGE_STYLE_ATTRIBUTE}]` +
...
export class StylesheetRefreshError extends Error {
  constructor(
    message: string,
    readonly code: "stylesheet-refresh-failed" | "stylesheet-refresh-timeout",
  ) {
    super(message);
...
export class PageStyles {
  ...
  async stage(
    ...
    const current = Array.from(document.querySelectorAll<HTMLElement>(PAGE_STYLE_SELECTOR));
...
const waitForStylesheet = (link: HTMLLinkElement, signal: AbortSignal): Promise<void> =>
