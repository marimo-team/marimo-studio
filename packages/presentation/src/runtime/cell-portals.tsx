...
type CellDiagnostic = Pick<ProjectionDiagnostic, "code" | "message" | "hint">;
...
const CellView = memo(function CellView({
  ...
  useEffect(() => {
    ...
    if (!runtimeReady || !bindingPresent || cell !== undefined || diagnostic !== undefined) {
      return;
    }
  ...
  const visibleOutputMessages = outputMessages.filter(
    (output) => output.data !== "" && output.data !== null,
  );
  const hasOutput = visibleOutputMessages.length > 0;
  const authoritativeOutputs =
    visibleOutputMessages.length > 0 ? visibleOutputMessages : outputMessages;
  ...
  const deliveryDiagnostic: CellDiagnostic | undefined =
    delivery === "timed-out"
      ? {
          ...
          message: `Cell ${JSON.stringify(host.cellName)} did not reach the browser.`,
          hint: "Wait for the notebook to settle, then reload the view.",
        }
      : undefined;
  const cellDiagnostic = executionDiagnostic({
    alias: host.cellName,
    disabled,
    failed,
    hasOutput,
  });
  const effectiveDiagnostic = diagnostic ?? deliveryDiagnostic ?? cellDiagnostic;
  ...
  }, [cell, effectiveDiagnostic, host, outputMime, outputMimesValue, state]);
  ...
  if (!cell) {
    ...
    return createPortal(
      <CellNotFound alias={host.cellName} developer={developer} diagnostic={effectiveDiagnostic} />,
      host,
    );
  }
