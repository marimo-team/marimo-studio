...
test("notebook query state preserves public values and removes transport keys", () => {
  const query = publicNotebookQuery(
    "?region=emea&region=apac&empty=&file=analysis.py&session_id=s_123456&access_token=secret&runtime=wasm&marimo_studio_client=client-123&marimo_studio_view=dashboard&marimo_studio_query_operation=query_1",
  );
