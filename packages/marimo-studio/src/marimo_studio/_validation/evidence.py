"""Browser observations and progressive validation evidence."""
...
class RuntimeStatusReport:
    ...
    def coherent(self) -> bool:
        ...
        if any(
            ...
            return False
        if any(
            not _safe_nonnegative_int(transition.sequence)
            or not _safe_nonnegative_int(transition.observed_at)
...
def _safe_nonnegative_int(value: int) -> bool:
    return 0 <= value <= (1 << 53) - 1


def _runtime_session_matches(runtime: str | None, session_id: str | None) -> bool:
    return bool(session_id) if runtime == "server" else session_id is None


@dataclass(frozen=True)
class ObservedProjectionInstance:
    """One browser-observed mount and its visible lifecycle state."""

    mount_id: str | None
    instance_id: str
    target: str
    runtime_cell_id: str | None
    phase: Literal["connecting", "loading", "stale", "ready", "missing", "error"]
    ...
    def ready(self) -> bool:
        return (
            self.phase == "ready"
            and self.mount_id is not None
            and self.runtime_cell_id is not None
...
class BrowserObservation:
    ...
    projection_instances: tuple[ObservedProjectionInstance, ...] = ()

    @property
    def projection_evidence_ready(self) -> bool:
        return all(item.ready for item in self.projection_instances)
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        if self.projection_instances:
            value["projection_instances"] = [
                item.to_dict() for item in self.projection_instances
            ]
        return value
...
class AnalysisReport:
    ...
    def handoff_ready(self) -> bool:
        ...
        if not self.browser_required:
            return not self.dynamic_browser_required
        ...
        return set(observations) == set(self.views) and all(
            ...
            and observation.projection_evidence_ready
        ...
        return {
            ...
            "stages": {
                ...
                "browser": {
                    ...
                    "required_for_dynamic_sites": self.dynamic_browser_required,
