...
def test_session_lifetime_cleanup_is_isolated_between_adapter_owners() -> None:
    async def exercise() -> None:
        ...
        lifetime_module._schedule_studio_session_close(first_session)
        lifetime_module._schedule_studio_session_close(second_session)
        first_timer = lifetime_module._STUDIO_SESSION_LIFETIMES[first_session].timer
        second_timer = lifetime_module._STUDIO_SESSION_LIFETIMES[second_session].timer
        assert first_timer is not None and second_timer is not None
...
def test_shared_manager_lifetime_waits_for_its_final_adapter_owner() -> None:
    async def exercise() -> None:
        ...
        assert lifetime_module._accept_studio_session(
            ...
            "s_target",
            session,
            lambda: first_callbacks.append(session),
            first_owner,
        )
        assert lifetime_module._accept_studio_session(
            manager,
            "s_target",
            session,
            lambda: second_callbacks.append(session),
            second_owner,
        )
        lifetime_module._schedule_studio_session_close(session)
...
def test_owner_close_serializes_with_native_terminal_notification() -> None:
    ...
    assert lifetime_module._accept_studio_session(
        ...
        "s_target",
        session,
        lambda: callbacks.append("second"),
        second_owner,
    )
...
def test_stale_ttl_generation_cannot_close_a_reaccepted_session() -> None:
    manager = _Manager()
    manager.ttl_seconds = None
    session = _Session()
    session.ttl_seconds = 60
    ...
    assert lifetime_module._accept_studio_session(
        ...
        "s_target",
        session,
        lambda: callbacks.append(session),
        owner,
    )
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        with observed_lock:
            closing = executor.submit(
                lifetime_module._close_studio_session,
                session,
                generation,
            )
...
def test_native_connector_rejects_a_lost_lifetime_owner() -> None:
    class Manager(_Manager):
        ttl_seconds = None
