...
import {
  ...
  type SourceName,
  type SourceRemote,
} from "../src/studio/source-remote.ts";
import {
  type SourceObserver,
  type SourceState,
  SyncedSource,
} from "../src/studio/source-sync.ts";
...
Deno.test("a clean external edit replaces the loaded source", async () => {
  ...
  assertEquals(result.documents, ["", "initial", "agent edit"]);
...
Deno.test("an older reconciliation cannot replace a newer result", async () => {
  ...
  assertEquals(result.documents.at(-1), "newest");
  assertEquals(result.states.at(-1)?.phase, "external");
});

Deno.test("the newest reconciliation applies after an older read finishes", async () => {
  const remote = new DeferredReadRemote();
  const result = observed();
  const source = new SyncedSource("index.html", remote, result.observer, 1);
  source.open("dashboard", { content: "initial", revision: "r0" });
...
Deno.test("a stale read failure cannot replace a newer result", async () => {
  const remote = new DeferredReadRemote();
  const result = observed();
  const source = new SyncedSource("index.html", remote, result.observer, 1);
  source.open("dashboard", { content: "initial", revision: "r0" });
...
Deno.test("a stale reconciliation cannot replace the current conflict", async () => {
  const remote = new DeferredReadRemote();
  const result = observed();
  ...
  assertEquals(result.states.at(-1)?.conflict, {
    local: "local edit",
    remote: { content: "current disk", revision: "r2" },
  });
});

Deno.test("a stale conflict read cannot replace a newer reconciliation", async () => {
  const remote = new DeferredReadRemote();
...
Deno.test("a failed view load never displays source from the previous view", async () => {
  const remote = new MemoryRemote();
  const result = observed();
  const source = new SyncedSource("index.html", remote, result.observer, 1);
  await source.load("dashboard");
  ...
  assertEquals(result.documents.at(-1), "");
  assertEquals(result.states.at(-1)?.phase, "error");
});

Deno.test("concurrent local and disk edits require an explicit resolution", async () => {
  const remote = new MemoryRemote();
  const result = observed();
  const source = new SyncedSource(
    "index.html",
  ...
  assertEquals(result.states.at(-1)?.phase, "conflict");
  assertEquals(result.states.at(-1)?.conflict, {
    local: "local edit",
    remote: { content: "agent edit", revision: "r2" },
  });
  source.useDisk();
  assertEquals(result.documents.at(-1), "agent edit");
});

Deno.test("keep mine writes against the latest disk revision", async () => {
  const remote = new MemoryRemote();
  const result = observed();
  ...
  assertEquals(remote.source.content, "body { color: red; }");
  assertEquals(result.states.at(-1)?.phase, "saved");
});

Deno.test("edits made during a save are written in revision order", async () => {
  ...
  assertEquals(remote.writes.length, 1);
  assertEquals(remote.writes[0].content, "first edit");
  remote.writes[0].resolve("r2");
  await Promise.resolve();
  assertEquals(remote.writes.length, 2);
  assertEquals(remote.writes[1].content, "latest edit");
  assertEquals(remote.writes[1].revision, "r2");
  remote.writes[1].resolve("r3");

  assertEquals(await autosave, true);
  assertEquals(await flush, true);
  assertEquals(source.hasConflict, false);
  assertEquals(source.hasPendingChanges, false);
});

Deno.test("a conflict keeps the latest editor text", async () => {
  const remote = new DeferredRemote();
  const result = observed();
  const source = new SyncedSource(
    "index.html",
  ...
  assertEquals(await saving, false);
  assertEquals(result.states.at(-1)?.conflict, {
    local: "latest edit",
...
Deno.test("dirty text without a loaded revision cannot be discarded", async () => {
  const remote = new MemoryRemote();
  ...
  assertEquals(await source.save(), false);
  assertEquals(source.hasPendingChanges, true);
  assertEquals(result.states.at(-1)?.phase, "error");
});

Deno.test("an external deletion clears clean source and reports repair", async () => {
  const remote = new MemoryRemote();
  const result = observed();
  const source = new SyncedSource("app.css", remote, result.observer, 60_000);
  await source.load("dashboard");
...
Deno.test("cancelling a staged load preserves a missing-source error", async () => {
  const remote = new MemoryRemote();
  const result = observed();
  const source = new SyncedSource("app.css", remote, result.observer, 60_000);
  await source.load("dashboard");
...
Deno.test("the filesystem event for an active save is not a conflict", async () => {
  const remote = new DeferredRemote();
  const result = observed();
  const source = new SyncedSource(
    "index.html",
  ...
  remote.writes[0].resolve("r2");
  await Promise.resolve();
  assertEquals(remote.writes[1].content, "newer content");
  assertEquals(remote.writes[1].revision, "r2");
  remote.writes[1].resolve("r3");

  assertEquals(await saving, true);
  assertEquals(source.hasConflict, false);
  assertEquals(source.hasPendingChanges, false);
});
