...
import { LayoutStorage } from "./layout-storage.ts";
...
export class LayoutController {
  ...
  reveal(surface: Surface): void {
    if (!visibleSurfaces(this.tree).includes(surface)) {
      const target = this.focused ?? this.compactSurface ??
        visibleSurfaces(this.tree)[0];
  ...
  private bindControls(): void {
    ...
    document.querySelectorAll<HTMLButtonElement>("[data-focus-surface]")
      .forEach((button) => {
        button.addEventListener("click", () => {
          const surface = button.dataset.focusSurface;
          if (!isSurface(surface)) {
            return;
          }
          this.focused = this.focused === surface ? null : surface;
          this.compactSurface = surface;
          this.commit();
        });
      });
    this.compactTabs.querySelectorAll<HTMLButtonElement>(
      "[data-compact-surface]",
    ).forEach((button) => {
      button.addEventListener("click", () => {
        const surface = button.dataset.compactSurface;
        if (isSurface(surface)) {
          this.compactSurface = surface;
          this.persist();
          this.render();
    ...
    document.querySelectorAll<HTMLDetailsElement>("[data-pane-menu]").forEach(
      (menu) => {
        menu.addEventListener("toggle", () => {
          const target = menu.dataset.paneMenu;
          if (menu.open && isSurface(target)) {
            this.renderPaneActions(menu, target);
          }
        });
      },
    );
  ...
  private renderPaneActions(
    ...
    renderPaneActions(menu, target, this.tree, ({ tree, compact }) => {
      ...
      this.compactSurface = compact ??
        (visible.includes(this.compactSurface)
          ? this.compactSurface
          : visible[0]);
      this.commit();
      this.closeMenus();
  ...
  private renderCompactTabs(compact: boolean, visible: Surface[]): void {
    ...
    this.compactTabs.querySelectorAll<HTMLButtonElement>(
      ...
      const surface = button.dataset.compactSurface;
      button.hidden = !isSurface(surface) || !visible.includes(surface);
      button.setAttribute(
        "aria-pressed",
        String(surface === this.compactSurface),
      );
    });
  }
  ...
  private closeMenus(): void {
    document.querySelectorAll<HTMLDetailsElement>("details[open]").forEach(
