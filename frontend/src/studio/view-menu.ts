...
export class ViewMenu {
  private readonly menu = document.querySelector<HTMLDetailsElement>(
    "[data-view-menu]",
  );
  private readonly add = document.querySelector<HTMLButtonElement>(
    "[data-new-view]",
  );
  private readonly form = document.querySelector<HTMLFormElement>(
    "[data-new-view-form]",
  );
  private readonly input = document.querySelector<HTMLInputElement>(
    "[data-new-view-name]",
  );
  ...
  render(
    ...
    list.replaceChildren(
      ...views.map((view) =>
        this.viewRow(view, current, removing, deleting, views.length > 1)
      ),
    );
  }
  ...
  showRemoval(name: string): void {
    ...
    document.querySelector<HTMLButtonElement>("[data-remove-view-submit]")
      ?.focus();
  }
  ...
  hideRemoval(focusView?: string): void {
    ...
    if (focusView) {
      document.querySelector<HTMLButtonElement>(
        `[data-remove-view="${CSS.escape(focusView)}"]`,
      )?.focus();
    }
  }
  ...
  setDeleting(deleting: boolean): void {
    ...
    document.querySelectorAll<HTMLButtonElement>("[data-remove-view]")
      .forEach((button) => {
  ...
  private bind(): void {
    ...
    document.querySelector<HTMLButtonElement>("[data-new-view-cancel]")
      ?.addEventListener("click", () => this.resetCreate());
    this.form?.addEventListener("submit", (event) => {
      event.preventDefault();
      this.actions.create(this.input?.value ?? "");
    });
    document.querySelector<HTMLButtonElement>("[data-remove-view-cancel]")
