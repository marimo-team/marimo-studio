import type { CellBindingConfig } from "./runtime-config.ts";
...
export const indexCells = <T extends CellIdentity>(
  cells: readonly T[],
): CellIndex<T> => {
...
export const resolveCellBinding = <T extends CellIdentity>(
  ...
  return binding.kind === "name"
    ? cells.byName.get(binding.value)
    : cells.byId.get(binding.value);
};
