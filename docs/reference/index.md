---
title: Reference
description: Exact Marimo Studio commands, configuration, Python APIs, and frontend provider contracts.
---

# Reference

Use the reference pages for exact command, configuration, API, and extension
contracts.

## Interfaces

- [CLI](cli.md) lists view, notebook, validation, and export commands.
- [Python API](python-api.md) defines saved-workspace and code-mode operations.
- [Configuration](configuration.md) defines notebook metadata, runtimes, routes,
  and view manifests.
- [Add another frontend](provider-api.md) defines the provider contract for
  creating, inspecting, and building frontend source.
