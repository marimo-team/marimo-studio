...
`iconify-icon` elements render named icons. Configure the hosting content
security policy with `script-src` access to jsDelivr, `connect-src` access to
the configured Iconify API, and `style-src` permission for the inline `<style>`
