...
- Audience: students and analysts reading about quadratic programs on their own.
- Goal: explain the standard form, the geometry, and duality while the reader
  changes the shape of `P` and the direction of `q`.
- Layout: the notebook's Markdown in a reading column, with the controls,
  `level_curves`, `solution_summary`, and a dual-value table sticky beside it.
  Narrow screens open with the controls and figure.
- Interaction: native `curvature` and `pull_direction` controls only. The
  Zero-Python export prepares every combination in `states.yaml`.
- The dual-value table reads the notebook's `solution` value. Keep analytical
  prose in notebook cells.

## Use the supplied Studio integration

`index.html` hosts the notebook cells the page shows by name: the reading
column's Markdown, the two controls, the figures, the solution summary, and the
takeaways. Keep, reorder, group, or replace those hosts as the page design
develops. Cell names remain stable Studio targets for the notebook cells.
...
`index.html` links the Inter and Newsreader stylesheets from jsDelivr, and its
inline `<style>` holds the page's tokens and layout. The fonts require browser
network access and a hosting content security policy with `style-src` and
`font-src` access to jsDelivr. The stylesheet's fallback fonts keep the page
