...
Build a focused athlete explorer for analysts and curious readers. Use Svelte
with Mosaic and vgplot as the primary visualization system. Seed browser DuckDB
from the projected `athlete_facts` Arrow data, then keep filters and crossfilter
selections local to the page. Label medal counts as athlete medal awards. Use an
Athlete Logbook aesthetic from `DESIGN.md`: warm linen, flat white analytical
surfaces, sharp corners, and one Strava-orange signal.

## Use the supplied Studio integration

`src/App.svelte` observes `athlete_facts` as an Arrow table.
`src/lib/athlete-explorer.ts` loads that stream into DuckDB-WASM and owns the
Mosaic controls, crossfilter, plots, scorecards, and roster table.
